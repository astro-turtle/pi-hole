...
displayFinalMessage() {
    ...
    if [[ "${INSTALL_WEB_INTERFACE}" == true ]]; then
        ...
        additional="View the web interface at https://pi.hole/admin or https://${IPV4_ADDRESS%/*}/admin\\n\\nYour Admin Webpage login password is ${pwstring}"
    fi
...
main() {
    ...
    if [[ "${EUID}" -eq 0 ]]; then
        ...
        if is_command sudo ; then
            ...
            if [[ "$0" == "bash" ]]; then
                ...
                exec curl -sSL https://raw.githubusercontent.com/astro-turtle/pi-hole/master/automated%20install/basic-install.sh | sudo bash "$@"
            else
    ...
    if [[ "${useUpdateVars}" == false ]]; then
        # If the Web interface was installed,
        if [[ "${INSTALL_WEB_INTERFACE}" == true ]]; then
            printf "  %b View the web interface at https://pi.hole/admin or https://%s/admin\\n\\n" "${INFO}" "${IPV4_ADDRESS%/*}"
        fi
